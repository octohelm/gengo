...
func New(pkg string, imports namer.ImportTracker) *Dumper {
	return &Dumper{
...
func (d *Dumper) TypeLit(tpe reflect.Type) string {
	if tpe.PkgPath() != "" {
		return d.namer.Name(gengotypes.Ref(tpe.PkgPath(), tpe.Name()))
	}
...
func (d *Dumper) ValueLit(in interface{}, optFns ...ValueLitOptFn) string {
	...
	case reflect.Struct:
		buf := bytes.NewBufferString(d.TypeLit(tpe))
		buf.WriteString(`{`)
	...
	case reflect.Slice, reflect.Array:
		...
		for i := 0; i < rv.Len(); i++ {
			...
			buf.WriteString(d.ValueLit(rv.Index(i), optFns...))
			buf.WriteString(",")
			buf.WriteString("\n")
		}
